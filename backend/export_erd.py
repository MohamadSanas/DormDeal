...
cols AS (
    ...
    SELECT array_to_string(
                      array_agg(CONCAT('{"name":"', conf.name, '","value":"', replace(conf.setting, '"', '\"'), '"}')),
                      ',') AS config_metadata
    ...
    SELECT array_to_string(array_agg(CONCAT('{"schema":"', replace(schema_name, '"', ''), '"',
                                            ...
                                            ',"expression":"', replace(replace(check_expr, '"', '\\"'), '\n', ' '),
...
def main():
    print("Connecting to Neon PostgreSQL to extract DrawSQL metadata...")
    with engine.connect() as conn:
        result = conn.execute(text(sql)).fetchone()
        json_data = result[0]
        with open("output.json", "w", encoding="utf-8") as f:
            f.write(json_data)
        print("SUCCESS: 'output.json' has been generated successfully!")
